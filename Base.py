...
class MyAviary(BaseAviary):
    ...
    def _computeObs(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            # return obs
            ############################################################
            #### OBS SPACE OF SIZE 12s
            return np.hstack([obs[0:3], obs[7:10], obs[10:13], obs[13:16], self.target_position]).reshape(15,)
            ############################################################
        else:
    ...
    def _computeReward(self):
        ...
        att_base = np.array([0, 0, 0])                      
        vel_base = np.array([0, 0, 0])
                              
        pos_err = np.linalg.norm(pos - self.target_position)
        att_err = np.linalg.norm(att - att_base)
#         vel_err = np.linalg.norm(vel - vel_base)
        act_err = np.linalg.norm(action - action_base)
        ...
        reward = 0.1 - w_pos * np.sum(pos_err) - w_att * np.sum(att_err) - w_der * da - w_act * act_err
